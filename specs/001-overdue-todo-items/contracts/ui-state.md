...
- Always returns `false` for completed items regardless of `dueDate`
- Always returns `false` when `dueDate` is absent
...
2. Render an overdue badge element adjacent to the due date line, containing:
   - A text label (e.g., "Overdue") — required for non-colour signal
